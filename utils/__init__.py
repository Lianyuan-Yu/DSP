...
def test_all_case(task, net, ids_list, num_classes, patch_size, stride_xy, stride_z, test_save_path=None):
    for data_id in tqdm(ids_list):
        image, labeli = read_data(data_id, task=task, normalize=True)
        ...
        #labeli = sitk.GetImageFromArray(labeli.astype(np.float32))
        #sitk.WriteImage(image, f'{test_save_path}/{data_id}_img.nii.gz')
        #sitk.WriteImage(labeli, f'{test_save_path}/{data_id}_gt.nii.gz')
    ...
    for x in range(sx):
        ...
        for y in range(sy):
            ...
            for z in range(sz):
                ...
                #y1 = net(test_patch)
                y1 = net(test_patch, use_prototype=False)[1]

...
if __name__ == '__main__':
    txt_path0 = "./logs1/" + args.exp + "/evaluation_res1.txt"
    fw = open(txt_path0, 'w')

    for epoch_num in range(0,351):
        pred0 = os.path.join("./logs1",args.exp, "predictions", f"epoch={epoch_num}")
        if not os.path.exists(pred0):
            continue

        ids_list = read_list(args.split, task=args.task)
        results_all_folds = []

        print('\n epoch =', epoch_num, "Evaluating...")
        for fold in range(1, args.folds + 1):
            ...
            for idx, data_id in enumerate(tqdm(ids_list)):
                pred = read_nifti(os.path.join("./logs1",args.exp, "predictions", f"epoch={epoch_num}", f'{data_id}.nii.gz'))
                lb_path = os.path.join(config.save_dir, 'npy', f'{data_id}_label.npy')
                label = np.load(lb_path)

                padding_flag = label.shape[0] < config.patch_size[0] or label.shape[1] < config.patch_size[1] or \
        ...
        fw.write(f'epoch = {epoch_num}; Final Avg Dice: ' + str(round(results_folds_mean.mean(0).mean(0)[0], 2)) + '\n')
        fw.write('Final Avg Jaccard: ' + str(round(results_folds_mean.mean(0).mean(0)[1], 2)) + '\n')
        fw.write('Final Avg HD95: ' + str(round(results_folds_mean.mean(0).mean(0)[2], 2)) + '\n')
        fw.write('Final Avg ASD: ' + str(round(results_folds_mean.mean(0).mean(0)[3], 2)) + '\n')

        print('Final Avg Dice: ' + str(round(results_folds_mean.mean(0).mean(0)[0], 2)))
        print('Final Avg Jaccard: ' + str(round(results_folds_mean.mean(0).mean(0)[1], 2)))
        print('Final Avg HD95: ' + str(round(results_folds_mean.mean(0).mean(0)[2], 2)))
        print('Final Avg ASD: ' + str(round(results_folds_mean.mean(0).mean(0)[3], 2)))
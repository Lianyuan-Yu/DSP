...
def analyze_models(model_1, model_2, device='cuda'):
    model_1, model_2 = model_1.to(device), model_2.to(device)
    # --- Params ---
    p1 = sum(p.numel() for p in model_1.parameters()) / 1e6
    p2 = sum(p.numel() for p in model_2.parameters()) / 1e6
    print(f"[Model_1] Params: {p1:.2f}M | [Model_2] Params: {p2:.2f}M")
...
if __name__ == '__main__':
    ...
    analyze_models(model_1, model_2)
    ...
    for epoch_num in range(args.max_epoch + 1):
        ...
        t1 = time.time()
        print(f"Training Time per Epoch (Trt): {t1 - t0:.2f} s")
...
###################################################################################
# LA_8:
# bs = 4, lr = 0.01, self.n_filters = 16, one optimizer model_1,2: VNet:
#proto_mom=0.99, temp=1, no_pro; loss = loss_1,0.8*2; L_unsup_1 = L_unsup_1_y + L_unsup_1_pk; loss_2 = loss_2_y + loss_2_pk; loss_1 = L_deno_1_l + 0.2 * mu_2 * L_deno_1_u
#train_8, [xl_epoch=40: loss_1,2_y; epoch=310: loss], model_2 [y_pk_l,y_pk_u] [image_l,image_u], max_epoch=350, test_every_best: %
#Final Avg Dice: 90.65; Final Avg Jaccard: 82.95; Final Avg HD95: 5.25; Final Avg ASD: 1.65 ################


# BraTS2019_25:
# bs = 4, lr = 0.01, self.n_filters = 16, one optimizer model_1,2: VNet:
#proto_mom=0.99, temp=1, no_pro; loss = loss_1,0.7*2; L_unsup_1 = L_unsup_1_y + L_unsup_1_pk; loss_2 = loss_2_y + loss_2_pk; loss_1 = L_deno_1_l + 0.01 * mu_2 * L_deno_1_u
#train_6, [xl_epoch=70: loss_1,2_y; epoch=280: loss], model_2 [y_pk_l,y_pk_u] [image_l,image_u], max_epoch=350, test_every_best: %
#Final Avg Dice: 84.16; Final Avg Jaccard: 74.68; Final Avg HD95: 9.58; Final Avg ASD: 2.45 ###############
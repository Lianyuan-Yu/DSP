...
class TimeStepEmbedding(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, normalization='none'):
        ...
        self.temb.dense = nn.ModuleList([
            torch.nn.Linear(self.embed_dim_in, self.embed_dim_out),
            torch.nn.Linear(self.embed_dim_out, self.embed_dim_out),
        ])
...
class Encoder_denoise(nn.Module):
    def __init__(self, n_filters, normalization, has_dropout, dropout_rate):
        ...
        self.dropout = nn.Dropout3d(p=dropout_rate, inplace=False)  # inplace=True
...
class DiffVNet1(nn.Module):
    def __init__(self, n_channels=3, 
                 n_classes=2, 
                 n_filters=16, normalization='none', has_dropout=False, dropout_rate=0.5):
        super(DiffVNet1, self).__init__()
        #self.n_classes = n_classes
        self.time_steps = 1000

        self.denoise_model = DenoiseModel(n_classes, n_channels, n_filters, normalization,
                                          has_dropout, dropout_rate)
    ...
    def forward(self, image=None, x=None, pred_type=None, step=None):
        ...
        if pred_type == "q_sample":
            ...
            features= self.denoise_model.encoder(x, temb)
            return self.denoise_model.decoder(features)
...
class DiffVNet2(nn.Module):
    def __init__(self, n_channels=3, 
                 n_classes=2, 
                 n_filters=16, normalization='none', has_dropout=False, dropout_rate=0.5):
        super(DiffVNet2, self).__init__()
        self.time_steps = 1000
